                ...
                <form class="form-horizontal" id="addForm" role="form" action="action/add_action.php" method="post">
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-md-4 input-group telNumber">    
                        ...
                        <input type="text" class="form-control " name="client_tel" id="client_tel" value="<?php echo isset($_SESSION['client_tel']) ? $_SESSION['client_tel'] : ""; ?>" onkeypress="if ( isNaN( String.fromCharCode(event.keyCode) )) return false;"  onfocusout="is_phonenumber(this)" onkeyup="formatTel(this)" oninput="if (this.value.length > this.maxLength) this.value = this.value.slice( this.maxLength, this.value - 1 );" maxlength="11"/>
                        
                        <?=isset($_SESSION['error']['empty_clientTel']) ? '<small class="field-msg error"  >'.  $_SESSION['error']['empty_clientTel'] . '</small>' : "" ?>
                        <?=isset($_SESSION['error']['invalidTel']) ? '<small class="field-msg error"  >'.  $_SESSION['error']['invalidTel'] . '</small>' : "" ?>
                      </div>
                      ...
                      <label for="staff_card" class="col-md-3 control-label">Nhóm:</label>
                        <div class="col-md-4">
                          <select name="client_group" id="client_group" class="form-control"  >
                            ...
                            <?=isset($_SESSION['error']['empty_clientGroup']) ? '<small class="field-msg error"  >'.  $_SESSION['error']['empty_clientGroup'] . '</small>' : "" ?>
                        </div>
                  </div>
...
<script type="text/javascript">
  ...
  $('.clearAll').click( function(){
      ...
      // $('select#client_group option').each(function(){
      //     if($(this).is(':selected'))
      //     {
      //       $(this).prop('selected',false);
      //     }
      // });//(1)
      // $('select#client_group option[class="default"]').prop('selected', true);

      $("select#client_group option:selected").prop("selected", false);//(2)
      $('select#client_group option[class="default"]').prop('selected', true);
