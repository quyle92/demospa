...
class clsOrder_KTV {
	   ...
	   //var_dump ( $_FILES['files']['name'] );
		...
		{	$i = 0;
	        ...
	        var_dump($updated_img_arr );
		 	...
		 	if ( !empty( $uploaded_img )  )
		 	{
		 		$sql = "UPDATE [tblDMNhanVien] SET SourceHinhAnh = '$updated_img_arr' WHERE MaNV = '$maktv'";
		 		$rs = $this->conn->query($sql);
	...
	public function deletePics( $maktv,  $pic_item_arr )
	{
		...
		$sql = "UPDATE [tblDMNhanVien] SET [SourceHinhAnh] = '$img_arr_updated' where MaNV ='$maktv'";
		try
		{
			$rs_1 = $this->conn->query($sql);
